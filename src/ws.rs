...
use crate::types::{BoostMessageInvoice, WsPacket, Video, VideoStatus};
...
impl State {
    async fn apply(mut self, msg: WsPacket, db: Arc<RedisMultiplexed>) -> Result<(Self, Action), MyError> {
        ...
        match (&mut self.room, &msg) {
            (ref mut self_room @ None, WsPacket::Join { room }) => {
                match Video::get(&db, room.clone()).await? {
                    Some(Video { status: VideoStatus::Live{ .. }, .. }) | Some(Video { status: VideoStatus::Scheduled { ..} ,.. }) => {
...
impl<T: 'static + error::Error + Send> From<T> for MyError {
    fn from(other: T) -> Self {
        MyError { err: Box::new(other) }
    }
}

async fn handle_connection(db: Arc<RedisMultiplexed>, btcpay_client: Arc<BTCPayClient>, config: Arc<Config>, raw_stream: TcpStream, addr: SocketAddr) -> Result<(), MyError> {
        ...
        while let Some(Ok(msg)) = incoming.next().await {
            ...
            match resp.1 {
                ...
                Action::Subscribe(room) => {
                    ...
                    let packet = WsPacket::AssignedUsername { username: id.clone() };
                    outgoing.send(Message::Text(serde_json::to_string(&packet)?)).await?;

                    let receive_from_others = rx.map(|msg| Ok(Message::Text(msg))).forward(outgoing.clone());
                    tokio::spawn(receive_from_others);
                },
                Action::Broadcast(message) => {
                    let packet = WsPacket::ServerMessage { from: id.clone(), message, extra: None };
                    let _: () = redis::Cmd::publish(state.room.as_ref().ok_or(MyError::empty())?, &serde_json::to_string(&packet)?)
                ...
                Action::CreateInvoice(amount, message) => {
                    ...
                    let webhook_data = BoostMessageInvoice { id: invoice.id.clone(), message, from: id.clone(), room: state.room.clone().ok_or(MyError::empty())? };
...
pub async fn start<S: tokio::net::ToSocketAddrs + std::fmt::Display>(
    ...
    while let Ok((stream, addr)) = listener.accept().await {
        tokio::spawn(handle_connection(db.clone(), btcpay_client.clone(), config.clone(), stream, addr));
    }
}
