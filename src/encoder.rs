...
impl Encoder {
    pub async fn new(id: String, global_config: &Config) -> Result<Self, EncoderError> {
        ...
        let path = Path::new(&global_config.storage_dir).join("recordings").join(&id).with_extension("flv");
        if !path.exists() {
            return Err(EncoderError::MissingInput);
        }

        let meta = VideoProbe::new(path.as_path())?.probe()?;
        let (video_width, video_height, video_framerate) = meta.iter().filter_map(|a| match a {
            NginxMeta::Video { width, height, frame_rate, .. } => Some((*width, *height, *frame_rate)),
            _ => None
        }).nth(0).unwrap();
        let duration = meta.iter().filter_map(|a| match a {
    ...
    pub async fn encode(self) -> Result<EncoderResult, EncoderError> {
        ...
        let output_dir = Path::new(&self.global_config.storage_dir).join("encoded").join(&self.id);
        ...
        for (id, vp9_config) in self.config.vod.vp9 {
            ...
            let fps = min(self.video_framerate, vp9_config.max_fps.unwrap_or(self.video_framerate));
            ...
            debug!("VP9 {} ({}): {} -> {}", id, workdir.display(), self.path.display(), output.display());
            for mut cmd in VP9::commands(vp9_config.bitrate, vp9_config.height, fps, vp9_config.audio_channels, 48000, &self.path, &output, &workdir)? {
                trace!("{:?}", cmd);
        ...
        for (id, h264_config) in self.config.vod.h264 {
            ...
            let fps = min(self.video_framerate, h264_config.max_fps.unwrap_or(self.video_framerate));
            ...
            debug!("H.264 {} ({}): {} -> {}", id, workdir.display(), self.path.display(), output.display());
            for mut cmd in H264::commands(h264_config.bitrate, h264_config.height, fps, h264_config.audio_channels, 48000, &self.path, &output, &workdir)? {
                trace!("{:?}", cmd);
...
trait Codec {
    fn commands(bitrate: usize, height: usize, fps: f32, audio_channels: usize, audio_sampling: usize, input: &Path, output: &Path, workdir: &Path) -> Result<Vec<Command>, EncoderError>;
}
...
mod test {
    ...
    fn test_encoder_decode_toml() {
        let result: EncodingConfiguration = toml::from_str(r#"
            [vod.vp9]
